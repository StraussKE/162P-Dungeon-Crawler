...
def outOfBounds (x_axis, y_axis, bounds, movement):
...
def tooClose (dungeon, current_floor, bounds, x_axis, y_axis, target_object):
    if (outOfBounds(x_axis, y_axis, bounds, NORTH) == False and checkMove(dungeon, current_floor, x_axis, y_axis, NORTH, target_object) == True):
        return true
    if (outOfBounds(x_axis, y_axis, bounds, WEST) == False and checkMove(dungeon, current_floor, x_axis, y_axis, WEST, target_object) == True):
        return true
    if (outOfBounds(x_axis, y_axis, bounds, SOUTH) == False and checkMove(dungeon, current_floor, x_axis, y_axis, SOUTH, target_object) == True):
        return true
    if (outOfBounds(x_axis, y_axis, bounds, EAST) == False and checkMove(dungeon, current_floor, x_axis, y_axis, EAST, target_object) == True):
        return true
    return false
...
def intInput(MIN_INPUT, MAX_INPUT):
    invalid = True
    while invalid:
        try:
            userInput = int(input())
